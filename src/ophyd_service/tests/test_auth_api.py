...
from ophyd_service.authorization._defaults import _DEFAULT_ROLES
...
def test_api_admin_auth_principal_apikey_01(
    ...
    resp5 = request_to_json(
        ...
        json={"expires_in": 900, "scopes": ["read:status", "read:devices"]},
        token=token,
    )
    ...
    assert set(resp5b["scopes"]) == {"read:status", "read:devices"}

    # Get an API key for the user (fixed scopes outside the scopes of the user)
    resp6 = request_to_json(
        "post",
        f"/auth/principal/{principals['alice']}/apikey",
        json={"expires_in": 900, "scopes": ["admin:apikeys", "read:status", "read:devices"]},
        token=token,
    )
    assert "detail" in resp6
